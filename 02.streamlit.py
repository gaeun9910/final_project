...
# 대시보드 제목
st.markdown("""
<h1 style='font-size:58px; text-align:center; margin-bottom:10px;'>
...
selected_dates = st.sidebar.date_input("", value=(available_dates[0], available_dates[-1]), min_value=available_dates[0], max_value=available_dates[-1])

st.sidebar.markdown("<label class='sidebar-label'>⏰ 시간 범위 선택</label>", unsafe_allow_html=True)
start_time, end_time = st.sidebar.slider("", value=(datetime.time(0, 0), datetime.time(23, 59)), format="HH:mm")

start_date, end_date = selected_dates
df = df[(df['date_only'] >= start_date) & (df['date_only'] <= end_date) & (df['time_only'] >= start_time) & (df['time_only'] <= end_time)]

# 필터 정보 표시
st.markdown(f"""
<div style="font-size:36px; margin-top:-5px; margin-bottom:30px; color: #333; text-align:center;">
...
icon_dict = {'energy_consumption': '⚡', 'humidity': '💧', 'pressure': '🌀', 'temperature': '🌡️', 'vibration': '💥'}
...
for i, metric in enumerate(metrics):
    with kpi_cols[i]:
        if metric in df_sorted.columns and not df_sorted[metric].dropna().empty:
            ...
            st.markdown(f"""
            <div style="border:2px solid #ccc; border-radius:12px; padding:10px 5px; text-align:center; background-color:#f9f9f9;">
                <div style="font-size:34px; font-weight:bold;">{icon} {metric.replace('_', ' ').title()}</div>
                <div style="font-size:52px; color:#2c3e50;">{last_value}</div>
            </div>
            """, unsafe_allow_html=True)
        else:
        ...
        if 'timestamp' in df.columns and 'maintenance_required' in df.columns:
            ...
            for i in range(len(full_df)):
                ...
                if status == 1 and last_status == 0 and last_ts is not None and in_range:
                    full_df.loc[i, 'diff_minutes'] = (now - last_ts).total_seconds() / 60
            ...
            bar_data['maintenance_required'] = bar_data['maintenance_required'].map({0: '정비 불필요', 1: '정비 필요'})

            fig_bar = px.bar(bar_data, x='maintenance_required', y='diff_minutes', color='failure_type', text='diff_minutes',
                             title='유지 보수별 소모 시간(분)', labels={'maintenance_required': '정비 필요 유무', 'failure_type': '고장 유형', 'diff_minutes': '누적 시간 (분)'},
                             color_discrete_sequence=color_sequence, height=600)
            fig_bar.update_layout(barmode='stack', title_font=dict(size=24), xaxis=dict(title='정비 필요 유무', tickfont=dict(size=16)),
                                  yaxis=dict(title='누적 시간 (분)', tickfont=dict(size=16), tickformat=',d'),
                                  legend=dict(font=dict(size=16)))
            fig_bar.update_traces(texttemplate='%{text:.0f}', textposition='outside', textfont=dict(size=20))
...
with col2:
    with st.container(border=True):
        if 'maintenance_required' in df.columns:
            ...
            df_pie['Status'] = df_pie.apply(lambda row: f'정비 필요 - {row["failure_type"]}' if row['maintenance_required'] == 1 else f'정비 불필요 - {row["failure_type"]}', axis=1)
            pie_data = df_pie['Status'].value_counts().reset_index()
            pie_data.columns = ['Status', 'Count']
            fig_pie = px.pie(pie_data, names='Status', values='Count', title='정비 여부 및 고장 유형별 비율', height=600,
                             color_discrete_sequence=color_sequence)
            fig_pie.update_traces(textfont_size=16)
            fig_pie.update_layout(title_font=dict(size=26), legend=dict(font=dict(size=20)))
...
with col_dot:
    with st.container(border=True):
        if 'timestamp' in df.columns and 'maintenance_required' in df.columns:
            ...
            fig_dot = px.scatter(dot_df, x='timestamp', y='maintenance_required', color='failure_type',
                                 title='날짜 및 시간에 따른 유지 보수',
                                 labels={'timestamp': '시간', 'maintenance_required': '정비 여부', 'failure_type': '고장 유형'},
                                 color_discrete_sequence=color_sequence, height=500)
            fig_dot.update_traces(marker=dict(size=10, symbol="diamond"))
            fig_dot.update_layout(title_font=dict(size=26),
                                  xaxis=dict(title='날짜 및 시간', tickfont=dict(size=18), tickformat='%Y-%m-%d %H:%M:%S'),
                                  yaxis=dict(title='정비 여부', tickfont=dict(size=18), tickvals=[0, 1], ticktext=['정상 (0)', '정비 필요 (1)']),
                                  legend=dict(font=dict(size=18)), legend_title=dict(font=dict(size=18)))
...
with col_gauge:
    with st.container(border=True):
        if 'predicted_remaining_life' in df_sorted.columns and not df_sorted['predicted_remaining_life'].dropna().empty:
            ...
            fig_gauge = go.Figure(go.Indicator(mode="gauge+number", value=last_life,
                                               number={'font': {'size': 60}},
                                               gauge={'axis': {'range': [0, 500], 'tickfont': {'size': 20}},
                                                      "bar": {"color": bar_color, "thickness": 1.0}}))
            fig_gauge.update_layout(title=dict(text="Remaining Life", font=dict(size=26), x=0.0), height=500, margin=dict(t=70))
